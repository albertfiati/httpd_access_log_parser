...
successfulConnections(){
	if [[ "$CHECKBLACKLIST" = "-e" ]]; then
		...
		grep '\.*1.1"\s\(2\|3\)\d\{2\}' $2 |  awk '{print "\t" $1 "\t\t" $9}' | sort | uniq -c | sort -r | awk '{print $2 "\t " $3 "\t " $1}' | head -$1 | while IFS=" " read -r -a line; do 
	        echo -e ${line[0]} "\t" ${line[2]} "\t " $(iPIsBlackListed ${line[0]})
		...
		echo -e IP Address "\t" Counts
		echo    -----------------------------------
		grep '\.*1.1"\s\(2\|3\)\d\{2\}' $2 |  awk '{print "\t" $1 "\t\t" $9}' | sort | uniq -c | sort -r | awk '{print $2 "\t " $1}' | head -$1
	fi
}
...
mostResultCodes(){
	if [[ "$CHECKBLACKLIST" = "-e" ]]; then
		...
		cat $2 |  awk '{print "\t" $9 "\t\t" $1}' | sort | uniq -c | sort -r | awk '{print $3 "\t " $2"\t "$1}' | head -$1 | while IFS=" " read -r -a line; do 
	        echo -e ${line[1]} "\t " ${line[0]} "\t " $(iPIsBlackListed ${line[0]})
	        #echo -e ${line[0]} "\t " ${line[1]} "\t" ${line[2]} "\t " $(iPIsBlackListed ${line[0]})
	    done
	else
		echo
		...
		echo    -----------------------------------
		cat $2 |  awk '{print "\t" $9 "\t\t" $1}' | sort | uniq -c | sort -r | awk '{print $2 "\t " $3}' | head -$1
...
failureConnections(){
	if [[ "$CHECKBLACKLIST" = "-e" ]]; then
		...
		echo -e Code "\t" IP Address
		echo    -----------------------------------
		grep '\.*1.1"\s\(4\|5\)\d\{2\}' $2 |  awk '{print"\t " $9"\t\t " $1}' | sort -r | uniq -c | sort -r | awk '{print $2 "\t " $3}' | head -$1
